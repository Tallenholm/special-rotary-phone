...
def _sentences(text: str) -> list[str]:
    return [s for s in (_normalized_sentence(x) for x in _SENTENCE_RE.split(text.strip())) if s]
...
def compute_section_delta(current: ExtractedSection, prior: ExtractedSection | None) -> SectionDelta:
    ...
    novelty = (
        sum(sentence not in prior_sentence_set for sentence in current_sentences) / len(current_sentences)
    ...
    return SectionDelta(
        ...
        word_count_change=(len(current_tokens) - len(prior_tokens)) / max(len(prior_tokens), 1),
        ...
        numeric_density_change=_numeric_density(current_tokens) - _numeric_density(prior_tokens),
        finance_term_density_change=_finance_term_density(current_tokens) - _finance_term_density(prior_tokens),
