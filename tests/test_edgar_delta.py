...
def test_numeric_and_finance_term_density_changes():
    ...
    prior_terms = 1
    current_terms = 3
    assert delta.finance_term_density_change == pytest.approx(
        (current_terms / 7) - (prior_terms / 6)
    )
...
def test_paragraph_multiset_counts_duplicate_changes():
    ...
    current = make_section(
        "item_3_legal_proceedings", "Same paragraph.\n\nNew paragraph."
    )
...
def test_missing_prior_returns_explicit_nan_features():
    current = make_section(
        "item_7a_market_risk", "Interest-rate risk increased to 5 percent."
    )
